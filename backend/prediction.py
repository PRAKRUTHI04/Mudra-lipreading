...
def getPrediction(response, puType, numClasses, modelPath, configPath, wordListPath, videoPath):
    ...
    #parser = argparse.ArgumentParser()
    # If called from root, paths should be correct if prefixed with lipreading/
    # If called from within lipreading/, they should be relative to current dir
    base_path = 'lipreading/' if os.path.exists('lipreading') else ''
    
    fullWordListPath = base_path + 'labels/' + wordListPath
    fullConfigPath = base_path + 'configs/' + configPath
    fullModelPath = base_path + 'models/' + modelPath
    ...
    fa = face_alignment.FaceAlignment(face_alignment.LandmarksType.TWO_D, device=deviceType)
    ...
    mean_face_path = base_path + 'preprocessing/words_mean_face.npy'
    mean_face_landmarks = np.load(Path(mean_face_path))
